...
Page({
    ...
    postComment(e) {
		if (this.data.comment.articleId && this.data.comment.commentContent != '' && typeof (this.data.comment.commentContent) != "undefined") {
            let replyData = {
                content: this.data.comment.commentContent,
                articleId: this.data.comment.articleId
            }
            review(replyData).then(e => {
                if (e.code === 0) {
                    this.setData({
                        autoFocus: false,
						[`comment.commentContent`]:'',
